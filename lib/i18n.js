(function() {
  var I18n, assert, findTemplate, get, set, lookupKey,
      EmHandlebars, keyExists, emVersion;

  EmHandlebars = Ember.Handlebars;
  emVersion = (Ember.VERSION||'').split('-')[0];
  get = Ember.get;
  set = Ember.set;
  assert = Ember.assert;
  ...
  function eachTranslatedAttribute(object, fn) {
    ...
    var attrsObject = (object.hasOwnProperty('attrs') &&
                       parseInt(emVersion) >= 2) ?
                       object.attrs : object;

    for (var key in attrsObject) {
      isTranslatedAttributeMatch = key.match(isTranslatedAttribute);
      if (isTranslatedAttributeMatch) {
        var translation = attrsObject[key] == null ? null : I18n.t(attrsObject[key]);
