(function(window) {
  ...
  function buildCssClassAttr(cssClass) {
    if (typeof cssClass === 'string') {
      return ' class="%@"'.fmt(cssClass);
    }
  }

  EmHandlebars.registerHelper('t', function(key, options) {
    var attrs, context, data, elementID, result, tagName, view, cssClassAttr;
    context = this;
    attrs = options.hash;
    data = options.data;
    view = data.view;
    ...
    cssClassAttr = buildCssClassAttr(attrs.cssClass);
    ...
    result = '<%@ id="%@"%@>%@</%@>'.fmt(tagName, elementID, cssClassAttr, I18n.t(key, attrs), tagName);
