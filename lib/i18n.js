(function(window) {
  ...
  var attrHelperFunction = function(options) {
    function assertValue(value) {
      Ember.assert("Translated attributes must be strings, not %@".fmt(value), Ember.typeOf(value) === 'string');
    }

    var attrs = options.hash;

    Ember.assert("You must specify at least one hash argument to translateAttr", !!Ember.keys(attrs).length);

    var view = options.data.view;
    var ret = [];
    var ctx = this || window;
    var dataId = uniqueElementId();
    var dataIdAttribute = "data-bindtranslateattr-" + dataId + "='" + dataId + "'";
    ...
    // only add data-bindtranslateattr-xxx if there is a bound property
    if (hasBoundValue) {
      ret.push(dataIdAttribute);
    }
